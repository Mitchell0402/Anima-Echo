...
- **Visual Renovation Plan**: Static art is now vector-source first: SVG/vector files are canonical, and PNGs are Godot runtime exports. The 2026-06-26 vector batch generated 127 first-pass placeholder assets covering the previous 23 P0/P1/P2 files, older static mine/town/prop art, and shared UI skin/icon art. Runtime scene/UI wiring and human art review are still pending. [Spec](specs/visual-renovation-plan.md)
...
- TODO: Review the 127 first-pass generated vector placeholders in-engine. Promote only approved rows to `implemented`; revise or regenerate any asset that is too rough, too abstract, or incompatible with current scene scale.
...
- TODO: Wire and review the generated visual assets listed in [visual_assets/inventory.md](visual_assets/inventory.md). The static batch exists with Godot import files, SVG sources, and sidecars; catalog icons still need UI mapping, UI skin files need theme/wiring work, and `oxygen_pump.png` still needs scene sprite wiring.
...
- TODO: Add sidecars for the legacy player and gnoll animation sheets. They were intentionally excluded from the static vector batch because they are animated raster sheets.
- TODO: Keep `scripts/tools/generate_vector_assets.py` as the batch manifest for first-pass static art. If a generated placeholder is refined, update the SVG/generator and sidecar together so the runtime PNG remains reproducible.
- TODO: Connect the generated UI icons to runtime data. Warehouse slot icons now exist for every catalog item, and optional HUD/menu icons exist for warehouse, coin, oxygen, weight, and health. Add a catalog/icon mapping or `ItemDatabase` mapping, then update the UI to load these textures instead of code-drawn placeholders.
- TODO: Continue the visual renovation plan from review/wiring: validate the generated town map, mine tiles/decor, minecart, NPCs, and UI skin in-engine; fix `small_mine.tscn`'s broken `assets/gj/environment/stones_1.png` reference; then wire catalog icons, oxygen pump art, and themed UI skins. See [visual-renovation-plan](specs/visual-renovation-plan.md).
