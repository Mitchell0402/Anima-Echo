...
- Customer budget is consumed by `total_price`, not `unit_price`. The negotiation layer applies variance and preferred/timing bonuses, so a sale that was worth 12 in the tooltip may close at 11 or 14. The UI shows `base_price` for stability; the actual transaction amount is computed inside `CustomerShopService` and only appears in the toast. This matches the design "negotiation happens, then the deal is shown, then the player confirms" — but it is one more place where the number in the UI and the number in the transaction differ, and the documentation has to spell it out.
